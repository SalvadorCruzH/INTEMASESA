...
<@emasesa.favoritosInitScript custonNameSpace "p_p_id_com_liferay_asset_publisher_web_portlet_AssetPublisherPortlet_INSTANCE_dkzm_"/>
				...
				<#list entries as curEntry>
					 <#assign journalArticle = curEntry.getAssetRenderer().getAssetObject() />
					  <li class="ema-favoritos__item ema-favoritos__item--add">
                        ...
                        />
							<@emasesa.accesosDirectos themeDisplay.getUserId() themeDisplay.getScopeGroupId() curEntry.getClassPK() curEntry.getClassNameId() custonNameSpace journalArticle.getTitle() journalArticle.getUrlTitle() "" journalArticle.getDDMStructureKey()/>
					</li>
				</#list>
			</ul>
		</div>
