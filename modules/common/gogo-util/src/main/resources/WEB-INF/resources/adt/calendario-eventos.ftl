...
<#assign objetivos = eventosRegister.objetivo?eval/>
...
<script type="text/javascript">
		...
		$("#ema-calendario").simpleCalendar({
			...
			events: [
				...
				<#assign fechaFin = diaFin+"/"+mesFin+"/"+yearFin/>
				{
					...
					<#list objetivos as objetivo>
						<#if objetivo.eventoId == item.eventoId>
							<#if objetivo.literal != "">
								documentacion: '${objetivo.literal}',
							</#if>
						</#if>
					</#list>
				},
			...
			onDateSelect: function(date, events) {
				if (events && events.length > 0 && date > new Date()){
					if (events[0].asisteElEmpleado === "X"){
						añadirBoton(true);
					}else if (events[0].asisteElEmpleado === undefined){
						añadirBoton(false);
					}
				}
			}
	...
	function añadirBoton(asiteEmpleado){
		if (asiteEmpleado === true) {
			$('#btn-register').remove();
			var btn = $('<button id="btn-anular" class="btn btn-primary">Anular inscripción</button>');
			$('.ema-calendario__buttonRegister').append(btn);
		}else if(asiteEmpleado === false){
			$('#btn-anular').remove();
			var btn = $('<button id="btn-register" class="btn btn-primary">Inscribirse</button>');
			$('.ema-calendario__buttonRegister').append(btn);
		}
	}
	...
	$(document).on('click', '#btn-register, #btn-anular', function(){
