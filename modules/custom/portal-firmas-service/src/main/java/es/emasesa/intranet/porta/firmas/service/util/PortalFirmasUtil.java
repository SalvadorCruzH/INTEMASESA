...
public class PortalFirmasUtil {
    ...
    public void enviarPortalFirmas(Map<String, Serializable> workflowContext,  String documentSIGDID){
    	try {    		
	        ...
	        if(workflowContext.size()>0){
	            ...
	            workflowTaskId = GetterUtil.getString(values.get(EmasesaConstants.WORKFLOW_TASK_ID));
    ...
    private List<String> getNifConsjeroDelegado(Map<String, Serializable> workflowContext){
    	...
    	try {
	        ...
	        String valorCampoExpando = _portalFirmasServicesSettings.posicionIdConsejeroDelegado();
	        LoggerUtil.debug(LOG, "nombre campo expando: " + nombreCampoExpando);
	        LoggerUtil.debug(LOG, "valorCampoExpando: " + valorCampoExpando);
	
	        long companyId = GetterUtil.getLong((String) workflowContext.get(WorkflowConstants.CONTEXT_COMPANY_ID));
		    User user = _userLocalService.getUserByScreenName(companyId, valorCampoExpando);
		    
		    if(Validator.isNotNull(user)) {
		    	LoggerUtil.debug(LOG, "Se obtiene un usuario en liferay con el ScreemName = " + valorCampoExpando);
		    	...
		    	LoggerUtil.debug(LOG, "No se ha encontrado ningun usuario en liferay con el screem name: "  + valorCampoExpando);
		    }
