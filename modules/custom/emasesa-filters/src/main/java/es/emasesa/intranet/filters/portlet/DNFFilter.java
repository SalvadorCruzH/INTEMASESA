...
public class DNFFilter extends BaseFilter {
    ...
    protected void processFilter(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain) throws Exception {
        ...
        try {
            ...
            long rolSindicales = _DNFFormSettings.gestoresSindicalesId();
            LoggerUtil.debug(LOG, "obtiene el rol de formacion y sindicales: " + rolFormacion + "/" + rolSindicales);
            ...
            if (httpServletRequest.getRequestURL().toString().contains("/solicitud")) {
                if (idRol != rolFormacion && idRol != rolSindicales) {
                    ...
                    return;
                } else if ((idRol == rolFormacion || idRol == rolSindicales) && (today.isBefore(startDate) || today.isAfter(endDate))) {
                ...
                if (idRol != rolFormacion && idRol != rolSindicales) {
                    httpServletResponse.sendRedirect("/group/guest/inicio");
                    return;
                } else if (idRol == rolSindicales && (today.isBefore(startDate) || today.isAfter(endDate))) {
